...
- Commits: imperative present tense, capitalized, no trailing period (`Add expert list search`). One concern per commit.
...
└── Resources/                Assets.xcassets (Colors/, Icons/), Localizable.xcstrings
```
...
- Localization: all user-facing strings in `Localizable.xcstrings`. `Text("literal")` localizes, `Text(variable)` does not, so view helpers take `LocalizedStringKey`, not `String`. If the project syncs strings from the localization sheet (ACKLocalization), never hand-edit the generated files.
